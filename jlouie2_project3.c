...
void stringReverse(char * string, struct stack * s){
	char * reversed = malloc(sizeof(char)*((*s).max_size));
	for(int x = 0; x < (*s).max_size; x++){
		*(reversed + x) = 0;
	}
	printf("reversed:%s\n", reversed);
	...
	for(int j = 0; j < ((*s).max_size); j++){
		*(reversed + j) = pop(s);
	}
	*(reversed + (*s).max_size - 1) = '\0';
	printf("reversed:%s\n", reversed);
	free(reversed);
